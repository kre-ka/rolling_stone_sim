...
class Sim:
    def __init__(
        self, curve: Curve, arc_length_param_rtol=1e-3, arc_length_param_atol=1e-3
    ) -> None:
        self._curve = curve

        # curve steepness
        k = (
            sym.diff(self._curve.y, self._curve.t)
            / sym.Abs(sym.diff(self._curve.x, self._curve.t))
        ).simplify()

        # this is acceleration from steepness in g units
        # it handles the vertical line situation (infinite k)
        a = sym.Piecewise(
            (-sym.sign(k), sym.Eq(sym.Abs(k), sym.oo)), (-k / (1 + k**2) ** 0.5, True)
        ).simplify()
        self._a_t_f = sym.lambdify(self._curve.t, a)

        t_table, p_table, a_table = self._make_lookup_tables(
            arc_length_param_rtol, arc_length_param_atol
        )
    ...
    def _make_lookup_tables(self, rtol, atol):
        ...
        p_integrand = sym.sqrt(
            sym.diff(self._curve.x, self._curve.t) ** 2
            + sym.diff(self._curve.y, self._curve.t) ** 2
        )
        ...
        t_tab = [
            self._curve.t_span[0],
            (self._curve.t_span[0] + self._curve.t_span[1]) / 2,
            self._curve.t_span[1],
        ]
        ...
        def _expand_tables(t_tab, a_tab):
            ...
            a_test_true = [self._a_t_f(t_test[0]), a_tab[1], self._a_t_f(t_test[-1])]
            ...
            if any(
                [err > atol for err in abs_errors] + [err > rtol for err in rel_errors]
            ):
                t_tab_lower, a_tab_lower = _expand_tables(
                    [t_tab[0], t_test[0], t_test[1]],
                    [a_tab[0], a_test_true[0], a_test_true[1]],
                )
                t_tab_upper, a_tab_upper = _expand_tables(
                    [t_test[1], t_test[2], t_tab[2]],
                    [a_test_true[1], a_test_true[2], a_tab[2]],
                )
...
def plot_sim_results(
    ...
    e_lim = expand_limits(
        (
            min(e_total.min(), e_kin.min(), e_pot.min()),
            max(e_total.max(), e_kin.max(), e_pot.max()),
        ),
        0.05,
    )
    ...
    supported_features = {
        "e_total": (line_e_total_t, (t, e_total)),
    ...
    if animated:
        ...
        if data_interval < interval:
            ...
            if interval / data_interval % 1 != 0:
                print(
                    "Data rate is not integer multiply of animation rate. Animation rate will decrease accordingly."
                )
            frame_multiplier = ceil(interval / data_interval)
            interval = int(data_interval * frame_multiplier)
            frames = [
                i * frame_multiplier for i in range(ceil(len(t) / frame_multiplier))
            ]
