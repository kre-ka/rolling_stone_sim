...
def cos_sin_modeler(
    slider_params_override: None | Dict[str, Dict[str, float]] = None,
) -> CurveModeler:
    """
    ...
    f_params = dict(
        zip(("Ax Ay wx wy".split()), sym.symbols("A_x A_y w_x w_y", real=True))
    )
    x = f_params["Ax"] * sym.cos(f_params["wx"] * t)
    y = f_params["Ay"] * sym.sin(f_params["wy"] * t)
    slider_params = {
        "Ax": {"min": -3, "max": 3, "step": 0.1, "value": 0.3},
